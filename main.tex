...
\lstdefinestyle{python_black}{
    ...
    basicstyle=\Medium\UbuntuMonospace,
    ...
    showtabs=false,                  
    tabsize=2,
    emph={build, sample, run, rrt_extend}, % Custom highlighting
    emphstyle=\ttb\color{pureminimalistic@text@red},    % Custom highlighting style
}

\lstset{style=python_black}

\lstdefinelanguage{Grammar}
    {morekeywords={expr, funcname, local, constant, binary, unary, compare, stmt, if, binop, unaryop, boolop, compop, atom, compops, assign, return, logic_stmt, ast_expr, body, atoms},
    keywordstyle=\color{c4},
    sensitive=false,
    emph={sin, cos, uniform, gauss, x, y, goal, Add, Sub, Mult, Div, Mod, Pow, UAdd, USub, Not, Invert, And, Or, Eq, NotEq, Lt, LtE, Gt, GtE, Is, Inv}, % Custom highlighting
    emphstyle=\ttb\cb,    % Custom highlighting style
    }
...
\lstdefinelanguage{Params}
    {morekeywords={K, step, rewire, bias, α, a, k, rad},
    keywordstyle=\cb,
    sensitive=false,
    emph={uniform, x, random, y}, % Custom highlighting
    emphstyle=\ttb\cc,    % Custom highlighting style
    }
    
\lstnewenvironment{fslisting}
  {\lstset{language=Params,
           basicstyle=\Medium\UbuntuMonospace,
           breakatwhitespace=false,         
           columns=fullflexible}}{}   
...
\begin{frame}[plain]{}
  ...
  \centering
  \vfill
  \red{\fontsize{40}{50}\selectfont Many Problems Are Program Learning In Disguise}
  \vfill
\end{frame}
...
\begin{frame}[plain]{AutoML-Zero \white{\cite{real2020automl}}}
  ...
  % Eventually, a competitive modern classifier is evolved.
    \begin{figure}
        \centering
        \only<1>{\includegraphics[scale=0.37]{figures/automl_zero_main_fig_blur_0.png}}
        \only<2>{\includegraphics[scale=0.37]{figures/automl_zero_main_fig_blur_1.png}}
        \only<3>{\includegraphics[scale=0.37]{figures/automl_zero_main_fig.png}}
    \end{figure}
\end{frame}
...
\begin{frame}[fragile]\frametitle{Evolvable Python Programs}
  \begin{columns}
      ...
      \end{column}
      \begin{column}{.5\linewidth}
          \lstinputlisting[language=Python]{code_examples/rrt_over.py}
      \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]\frametitle{Evolvable Python Programs}
  \begin{columns}
      ...
      \lstinputlisting[language=Python]{code_examples/rrt_build.py}
      \end{column}
      \begin{column}{.55\linewidth}
      \lstinputlisting[language=Python]{code_examples/rrt_sample.py}
      \end{column}
      }
      \only<2>{
      \begin{column}{.45\linewidth}
      \lstinputlisting[language=Params]{code_examples/rrt_build.py}
      \end{column}
      \begin{column}{.55\linewidth}
      \lstinputlisting[language=Params]{code_examples/rrt_sample.py}
      \end{column}
      }
  \end{columns}
  \onslide<2->{
  \begin{table}[]
      ...
      \hline
      \multicolumn{1}{c}{\bf MD5 Hash}  & 
      \multicolumn{1}{c}{\bf \cb{\Large $\alpha$} (bias)}   &
      \multicolumn{1}{c}{\bf \cb{\Large $r_0$   } (step)}   &
      \multicolumn{1}{c}{\bf \cb{\Large $r_1$   } (rewire)} &
      \multicolumn{1}{c}{\bf \cb{\Large $K$   } (nodes)}    \\
      \hline
      \plnr{$abcde$} & \cb{1.0} & \cb{50 } & \cb{100} & \cb{1000} \\
      \plnr{$fghij$} & \cb{0.5} & \cb{100} & \cb{200} & \cb{1000} \\
      \end{tabular}
      \label{tab:params}
  \end{table}
  }
...
\begin{frame}[fragile]\frametitle{Evolvable Python Grammar}
  \lstinputlisting[language=Grammar]{code_examples/grammar.txt}
\end{frame}
...
\begin{frame}{\white{Future Work}}
  \begin{columns}[T]
      \begin{column}{.5\linewidth}
        \emphasis{Algorithms} 
        \begin{itemize}
            \onslide<2->{\item Advanced Route Planning}
            \onslide<2->{\item Advanced Sampling}
        \end{itemize}
        \vspace{0.5em}
        \emphasis{Environments} 
        \begin{itemize}
            \onslide<3->{\item High-Dimensions}
        \end{itemize}
        \vspace{0.5em}
        \emphasis{Optimization} 
        \begin{itemize}
            \onslide<4->{
            \item Alt. Pareto Evolution
            }
        \end{itemize}
      \end{column}
      \begin{column}{.5\linewidth}
        \begin{figure}
            \centering
            \includegraphics[width=0.9\linewidth, keepaspectratio]{figures/lavalle.png}
            \caption{\red{\cite{lavalle2006planning}}}
        \end{figure}
      \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[plain]{}
    ...
    \vfill
    % But *BECAUSE* there is no free lunch,
    %   Meta-learning and specialization are possible.
    \onslide<2->{\ce{Meta learning and specialization}}
\end{frame}
...
\centering
\vfill
{\fontsize{40}{50}\selectfont \red{Questions?}}
\vfill
\begin{figure}
    \centering
    \begin{overpic}[width=0.45\linewidth, keepaspectratio]{figures/meme4.jpg}
